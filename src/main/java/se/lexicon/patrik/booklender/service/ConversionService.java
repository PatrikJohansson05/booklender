...
import se.lexicon.patrik.booklender.entity.EntityFactory;
...
public class ConversionService extends EntityFactory {
    ...
    public Book dtoToBook(BookDto dto) {
        Book book = createBook(dto.getTitle(), dto.getMaxLoanDays(), dto.getFinePerDay(), dto.getDescription(), dto.isAvailable(), dto.isReserved());
        return book;
    }
    ...
    public LibraryUser dtoToUser(LibraryUserDto dto) {
        LibraryUser user = createLibraryUser(dto.getRegDate(), dto.getName(), dto.getEmail());
        return user;
    }
    ...
    public Loan dtoToLoan(LoanDto dto) {
        Loan loan = createLoan(dtoToUser(dto.getLoanTaker()), dtoToBook(dto.getBook()), dto.getLoanDate());
        return loan;
    }
